...
Passando invece alle pagine, NewActPage è la prima e contiene il form nel quale inserire le varie informazioni necessarie al successo dell'operazione. Il primo metodo è fillDates() e, come il nome suggerisce, è il responsabile della compilazione automatica delle date per l'utente. 

\begin{lstlisting}[frame=single]
private fillDates() {
	if (this.actEnd == undefined && this.prenStart == undefined && this.prenEnd == undefined) {
		...
		if (this.emptyPeriod >= 0) {
			this.prenEnd = actStart.subtract(this.emptyPeriod, 'd').format("YYYY-MM-DD");
		} else {
			this.prenEnd = actStart.subtract(1, 'd').format("YYYY-MM-DD");
		}
		let prenEnd = moment(this.prenEnd);
		if (this.reservationPeriod >= 0) {
			this.prenStart = prenEnd.subtract(this.reservationPeriod, 'd').format("YYYY-MM-DD");
		} else {
			this.prenStart = actStart.subtract(1, 'M').format("YYYY-MM-DD");
		}
		actStart = moment(this.actStart);
		this.maxPrenEnd = actStart.subtract(1, "d").format("YYYY-MM-DD");
		actStart = moment(this.actStart);
		this.maxPrenStart = actStart.subtract(1, "d").format("YYYY-MM-DD");
		if (moment(this.prenStart).isBefore(this.minPrenStart)) {
			this.prenStart = this.minPrenStart;
			this.savePeriods = false;
		} else if (moment(this.prenStart).isAfter(this.maxPrenStart)) {
			this.prenStart = this.maxPrenStart
			this.savePeriods = false;
		}
		if (moment(this.prenEnd).isBefore(this.prenStart)) {
			this.prenEnd = this.prenStart;
			this.savePeriods = false;
		} else if (moment(this.prenEnd).isAfter(this.maxPrenEnd)) {
			this.prenEnd = this.maxPrenEnd;
			this.savePeriods = false;
		}
	}
}
...
\begin{lstlisting}
	private countDays() {
		...
		let prenStart = moment(this.prenStart);
		let prenEnd = moment(this.prenEnd);
	...
	private submit(isRetryCall?: boolean): void {
		if (this.actStart != undefined && this.actEnd != undefined && this.prenStart != undefined && this.prenEnd != undefined && this.checkedTeachings.length > 0) {
			...
			if (this.actStart > moment(moment().year() + "01" + "31").format("YYYY-MM-DD")) {
				for (let t of this.checkedTeachings) {
			...
			this.ip.registerAct(
				new ActTeachings({
					...
					// An act which has less than one reservation day is considered a "special case". Hence don't save it.
					if (this.savePeriods) {
					...
					// Tutte le varie condizioni d'errore
				});
		} else {
